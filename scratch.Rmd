...
> It is important to note that this is not a policy or economic paper, nor will it attempt to delve deeply into macroeconomic theory. Such an analysis would fall beyond my expertise as a data visualisation enthusiast and political economy student. Whether the Shock was an unnecessary misstep by the Nixon administration, or an imperfect inevitability is decidedly beyond the scope of this demonstration, which is intended only to be a showcase for informative purposes.
...
This section is a technical explanation of packages, for fellow R enthusiasts. If you're more interested in the graphs, feel free to skip this section. Otherwise click here.
...
```{r echo=TRUE, fig.align="center", fig.height=6, message=FALSE, warning=FALSE}
...
I will also set a custom colour scale for use throughout different visualisations, for the sake of design continuity. The palettes are defined here, and displayed below. The colours and general design are inspired by vintage book covers by Pelican Books from the 1970s.
...
prod_wage <- read_csv("productivity_wage.csv") %>%
...
read_csv("productivity_wage.csv") %>%
  mutate_all(funs(str_replace(., "%", ""))) %>%
  mutate_all(funs(str_squish(.))) %>%
  transmute(
    Year = as.numeric(Year),
...
```{r message=FALSE, warning=FALSE, eval = FALSE}
plot_ceo_ratio <- read_csv("ceo_comp.csv") %>% 
  mutate(
...
## Inequality 2: Top 5% as share of total income {.tabset .tabset-fade .tabset-pills}

In a similar pattern, while in the post-war era, the top 5% of earners in America accounted for an unprecedentedly small proportion of total income, this proportion started to increase dramatically again, with the trend reversal beginning around 1971. Note that this data, taken from Saez and Piketty's original research, goes further back but is only available up to 1998. While more updated data is not readily available, judging from previous patterns, it wouldn't be unreasonable to surmise that this proportion has only increased since.

### Plot

```{r out.width='100%', echo=FALSE}
knitr::include_graphics("plots/plot_top5_incomeshare.png")
```

### Code

```{r message=FALSE, warning=FALSE, eval = FALSE}
saez <- read_csv("pikettysaez_table2.csv") 


plot_top5_incomeshare <- saez %>% 
  transmute(year, top5 = share_95_99+share_99_100) %>% 
  pivot_longer(
    cols = -year,
    names_to = "pop",
    values_to = "value"
  ...
  scale_y_continuous(labels = scales::percent)+
  labs(
    title = "Share of total income by top 5% earners 1917-1998",
    subtitle = "(with Loess smoothed trendline)",
    x = "Year",
    y = "% Share of total \n",
...
ggsave(plot = plot_top5_incomeshare,
       filename = "plots/plot_top5_incomeshare.png",
       h = 5,
       w = 10,
       type = "cairo-png")
```
...
```{r message=FALSE, warning=FALSE, paged.print=TRUE, echo = FALSE}
read_csv("pikettysaez_table2.csv") %>% 
  kbl() %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "100%", height = "500px")
